...
class WeatherController {
    ...
    handleSearch(event) {
        // console.log("Event is Fired " + event.key);
    ...
    updateUI(responseJSON){
        ...
        temperatureElement.innerHTML = `${responseJSON.main.temp} ${DEGREES_CELCIUS_SYMBOL}`;

        let weatherElement = document.querySelector(".current .weather");
        weatherElement.innerText = `${responseJSON.weather[0].main}`;

        let lowHighElement = document.querySelector(".current .hi-low");
        lowHighElement.innerHTML = `${responseJSON.main.temp_min} ${DEGREES_CELCIUS_SYMBOL} / 
        ${responseJSON.main.temp_max} ${DEGREES_CELCIUS_SYMBOL}`; 
    }
}
